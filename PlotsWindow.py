...
from SettingConstant import PlotType, PlotViewType
...
class PlotsWindow(BaseSubWindow):
    ...
    def make_plot(self):
        if self.builder.date_to is not None and self.builder.date_from is not None and self.builder.date_to < self.builder.date_from:
            messagebox.showwarning('Помилка','Дата кінця менша за дату початку')
            return
        plot_type = self.selected_plot_type.get()
        self.builder.add_plot_view_type(PlotViewType.BAR if self.selected_plot_view_type.get() =='Стовпчаста діаграма' else PlotViewType.PLOT)
        if plot_type == 'Графік кількості продажів':
            self.builder.add_plot_type(PlotType.PLOT_BY_DAY_CHECK_COUNT)\
            .build()
    ...
    def create_plot_type_menu(self):
        ...
        self.plot_type.grid(row=2, column=0,)


    def setup_layout(self):
        self.calendar_from.grid(row=0, column=0)
        self.calendar_to.grid(row=0, column=1)
        self.label_from.grid(row=1, column=0)
        self.label_to.grid(row=1, column=1)
        self.button_make_plot.grid(row=2, column=1)
